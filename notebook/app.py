...
Ruang Lingkup Tesis:
- 2 Role: Admin (full) | User (business insight)
- Fitur: Login, Dashboard Utama, Manajemen Data, Strategi Hunian & Harga, Logout
- Model: SARIMA | Evaluasi: RMSE + MAPE
- Library: pandas, numpy, matplotlib, seaborn, statsmodels, scikit-learn
- Visualisasi: Streamlit + Plotly

Instalasi:
    pip install streamlit pandas numpy statsmodels pmdarima scikit-learn scipy plotly
...
DB_PATH                = "sarima_app.db"
FORECAST_STEPS         = 6          # bulan ke depan
MIN_SEASONAL_TRAIN     = 30         # minimum data (bln) agar seasonal diaktifkan
FLAT_STD_THRESH        = 1.5        # threshold forecast dianggap flat
MIN_CYCLES             = 2
ACF_ALPHA              = 0.10
FALLBACK_M             = 12         # periode musiman default
SESSION_DURATION_HOURS = 24 * 30    # 30 hari

SESSION_KEY = "_sess_token"

LOGO_URL = "https://storage.googleapis.com/villasrus/public/images/logo/villasrus-311x256.webp"

# Palet warna per vila
VILLA_COLORS = [
...
def init_db():
    with get_db() as c:
        ...
        cur.execute("""CREATE TABLE IF NOT EXISTS villa_config (
            villa TEXT PRIMARY KEY,
            area  TEXT NOT NULL,
        ...
        cur.execute("""CREATE TABLE IF NOT EXISTS upload_log (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT, villa TEXT, data_type TEXT,
            filename TEXT, rows INTEGER,
            uploaded TEXT DEFAULT CURRENT_TIMESTAMP
        )""")

        cur.execute("""CREATE TABLE IF NOT EXISTS model_log (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT, villa TEXT, order_str TEXT,
            aic REAL, rmse REAL, mape REAL,
        ...
        # Seed villa default
        if not cur.execute("SELECT 1 FROM villa_config").fetchone():
            cur.executemany(
                "INSERT OR IGNORE INTO villa_config (villa,area,color) VALUES (?,?,?)",
                DEFAULT_VILLAS
            )
            c.commit()
...
def db_get_users() -> list[dict]:
    with get_db() as c:
        return [dict(r) for r in c.execute("SELECT id,username,role,created FROM users").fetchall()]
...
def db_load_villas() -> dict:
    with get_db() as c:
        rows = c.execute("SELECT villa,area,color FROM villa_config").fetchall()
    return {r["villa"]: {"area": r["area"], "color": r["color"]} for r in rows}

def db_save_villa(villa: str, area: str, color: str):
    with get_db() as c:
        c.execute("INSERT OR REPLACE INTO villa_config (villa,area,color) VALUES (?,?,?)",
                  (villa, area, color))
        c.commit()

def db_delete_villa(villa: str):
    with get_db() as c:
        c.execute("DELETE FROM villa_config WHERE villa=?", (villa,))
        c.execute("DELETE FROM raw_data WHERE villa=?", (villa,))
        c.execute("DELETE FROM models WHERE villa=?", (villa,))
        c.commit()
...
def db_save_data(villa: str, dtype: str, filename: str, content: str, rows: int) -> bool:
    ...
    with get_db() as c:
        existing = c.execute(
            "SELECT content FROM raw_data WHERE villa=? AND data_type=?",
            (villa, dtype)
        ).fetchone()

        if existing:
            try:
                df_old = _parse_csv(existing["content"])
                ...
                total_rows = rows
            c.execute(
                "UPDATE raw_data SET content=?,rows=?,filename=?,uploaded=CURRENT_TIMESTAMP "
                "WHERE villa=? AND data_type=?",
                (merged_content, total_rows, filename, villa, dtype)
            )
            merged = True
        else:
            c.execute(
                "INSERT INTO raw_data (villa,data_type,filename,content,rows) VALUES (?,?,?,?,?)",
                (villa, dtype, filename, content, rows)
            )
            merged = False
        c.commit()
    return merged
...
def db_delete_data(villa: str, dtype: str):
    with get_db() as c:
        c.execute("DELETE FROM raw_data WHERE villa=? AND data_type=?", (villa, dtype))
        c.commit()

def db_list_data() -> list[dict]:
    with get_db() as c:
        return [dict(r) for r in c.execute(
            "SELECT villa,data_type,filename,rows,uploaded FROM raw_data ORDER BY uploaded DESC"
        ).fetchall()]

def db_data_info(villa: str, dtype: str) -> dict | None:
    with get_db() as c:
        row = c.execute(
            "SELECT filename,rows,uploaded FROM raw_data WHERE villa=? AND data_type=?",
            (villa, dtype)
        ).fetchone()
    return dict(row) if row else None
...
def db_save_model(villa: str, info: dict):
    ...
    pkl_data = pickle.dumps(saveable)
    ...
    with get_db() as c:
        c.execute(
            "INSERT OR REPLACE INTO models (villa,pkl_data,order_str,aic,rmse,mape,trained_at) "
            "VALUES (?,?,?,?,?,?,CURRENT_TIMESTAMP)",
            (villa, pkl_data, order_str, aic,
             round(info.get("rmse", 0), 4),
             round(info.get("mape", 0), 4))
        )
        c.commit()

def db_load_model(villa: str) -> dict | None:
    with get_db() as c:
        row = c.execute("SELECT pkl_data FROM models WHERE villa=?", (villa,)).fetchone()
    if not row:
        return None
    try:
        return pickle.loads(row["pkl_data"])
    except Exception:
        return None

def db_model_exists(villa: str) -> bool:
    with get_db() as c:
        return bool(c.execute("SELECT 1 FROM models WHERE villa=?", (villa,)).fetchone())

def db_model_trained_at(villa: str) -> str:
    with get_db() as c:
        row = c.execute("SELECT trained_at FROM models WHERE villa=?", (villa,)).fetchone()
    return row["trained_at"] if row else "—"

def db_list_models() -> list[dict]:
    with get_db() as c:
        return [dict(r) for r in c.execute(
            "SELECT villa,order_str,aic,rmse,mape,trained_at FROM models ORDER BY trained_at DESC"
        ).fetchall()]

# Logs
def log_upload(username: str, villa: str, dtype: str, filename: str, rows: int):
    with get_db() as c:
        c.execute("INSERT INTO upload_log (username,villa,data_type,filename,rows) VALUES (?,?,?,?,?)",
                  (username, villa, dtype, filename, rows))
        c.commit()

def log_model(username: str, villa: str, order_str: str, aic: float, rmse: float, mape: float):
    with get_db() as c:
        c.execute("INSERT INTO model_log (username,villa,order_str,aic,rmse,mape) VALUES (?,?,?,?,?,?)",
                  (username, villa, order_str, aic, rmse, mape))
        c.commit()

def get_upload_log() -> list[dict]:
    with get_db() as c:
        return [dict(r) for r in c.execute(
            "SELECT * FROM upload_log ORDER BY uploaded DESC LIMIT 100"
        ).fetchall()]

def get_model_log() -> list[dict]:
    with get_db() as c:
        return [dict(r) for r in c.execute(
            "SELECT * FROM model_log ORDER BY trained DESC LIMIT 100"
        ).fetchall()]
...
def clean_occupancy(df: pd.DataFrame) -> pd.Series:
    """Normalisasi DataFrame mentah → Series okupansi harian 0–100."""
    df = df.copy()
...
def clean_revenue(df: pd.DataFrame) -> pd.Series:
    """Ekstrak revenue bulanan dari DataFrame finansial."""
    df = df.copy()
    date_col = _find_col(df, ["date", "tanggal", "tgl"]) or df.columns[0]
    df[date_col] = _parse_dates(df[date_col])
...
def load_all_data(_villa_cfg_json: str) -> tuple[dict, dict]:
    """Load & clean semua data vila. Return (occ_dict, fin_dict)."""
    villa_cfg = json.loads(_villa_cfg_json)
    occ_dict, fin_dict = {}, {}

    for villa in villa_cfg:
        # Ocupansi — coba dari financial dulu (lebih lengkap), fallback ke occupancy
...
def detect_m(monthly: pd.Series) -> tuple[int, str]:
    """Deteksi periode musiman via periodogram + ACF."""
    n = len(monthly)
    ...
    # Ambil power tertinggi per periode unik
    pp = {}
...
def compute_mape(actual: np.ndarray, predicted: np.ndarray) -> float:
    """MAPE standar — skip nilai actual mendekati 0."""
...
def run_adf_all(clean_occ: dict, villa_cfg: dict) -> tuple[dict, pd.DataFrame]:
    """Jalankan ADF untuk semua vila, return (villa_d, summary_df)."""
    villa_d, rows = {}, []
...
def train_sarima(series: pd.Series, d: int, m: int, color: str, title: str) -> dict:
    """Train SARIMA untuk satu vila. Return info dict."""
    monthly      = series.resample("MS").mean().dropna()
    ...
    # Auto ARIMA untuk order terbaik
    try:
        auto = auto_arima(
            train,
...
def make_forecast(info: dict) -> dict:
    """Buat forecast 6 bulan ke depan dari model yang sudah dilatih."""
...
def apply_base(fig: go.Figure, **extra) -> go.Figure:
    ...
    # Merge nested dicts (xaxis, yaxis) instead of overwriting
...
def chart_trend_all(clean_occ: dict, villa_cfg: dict) -> go.Figure:
    """Line chart semua vila, satu panel."""
    fig = go.Figure()
    for villa, series in clean_occ.items():
...
def chart_bar_mean(clean_occ: dict, villa_cfg: dict) -> go.Figure:
    """Bar chart rata-rata okupansi per vila."""
    data = sorted([
...
def chart_model_fit(info: dict) -> go.Figure:
    """Grafik train/test/predicted — evaluasi model."""
...
def chart_forecast(info: dict, fore: dict) -> go.Figure:
    """Grafik forecast 6 bulan ke depan."""
    monthly = info["monthly"]
    color, title = info["color"], info["title"]
...
def chart_scatter_occ_rev(clean_occ: dict, clean_fin: dict, villa_cfg: dict) -> go.Figure | None:
    """Scatter plot okupansi vs revenue per vila."""
...
def chart_residual(info: dict) -> go.Figure:
    """3-panel diagnostik residual: time series, histogram, Q-Q."""
    ...
    fig.update_layout(
        title=dict(
            text=f"{title} — Diagnostik Residual | {lb_lbl} (p12={p12:.3f}) | Shapiro p={p_n:.3f} {n_lbl}",
...
def status_badge(value: float, thresholds: tuple = (75, 50)) -> tuple[str, str, str]:
    """Return (icon, label, bg_color) berdasarkan nilai okupansi."""
...
def period_filter(clean_occ: dict, key: str) -> tuple:
    """Widget filter periode, return (date_start, date_end)."""
    all_dates = []
...
# ══════════════════════════════════════════════════════════════
# HALAMAN — DASHBOARD UTAMA (Admin + User)
# ══════════════════════════════════════════════════════════════
...
def page_dashboard(clean_occ: dict, clean_fin: dict, villa_cfg: dict):
    ...
    with c3:
        kpi_card("Model Prediksi Siap", str(n_with_model), "villa terlatih",
    ...
    for villa, cfg in villa_cfg.items():
        mi = db_load_model(villa) if is_admin else None
        ...
        if df_prev is not None:
            ...
            with col_del:
                if st.button("🗑️ Hapus Data", type="secondary", use_container_width=True,
                    ...
                    db_delete_villa(prev_villa) if False else None  # hanya hapus data, bukan vila
                    load_all_data.clear()
...
# ══════════════════════════════════════════════════════════════
# HALAMAN — STRATEGI HUNIAN & HARGA (Admin: full | User: insight)
# ══════════════════════════════════════════════════════════════
...
def page_strategi(
    ...
    # Tab berbeda untuk admin vs user
    if is_admin:
        tabs = st.tabs([
            "🔭 Prediksi & Strategi",
            "🔬 Analisis Teknis (EDA)",
    ...
    with tabs[0]:
        if not sarima_models:
            st.info("⏳ Model prediksi belum tersedia. Hubungi administrator untuk melatih model.")
            return
        ...
        for villa in selected_villas:
            if villa not in sarima_models or villa not in fore_info_all:
                continue
            info = sarima_models[villa]
            fore = fore_info_all[villa]
            fore_mean   = fore["fore_mean"]
            avg_f       = fore_mean.mean()
            peak_month  = fore_mean.idxmax().strftime("%B %Y")
            low_month   = fore_mean.idxmin().strftime("%B %Y")
            icon, label, bg = status_badge(avg_f)

            # Rekomendasi strategi
            if avg_f >= 75:
                rec = f"Pertimbangkan **menaikkan tarif** saat puncak hunian di **{peak_month}**."
                rec_type = "success"
            elif avg_f >= 50:
                rec = f"Perkuat **promosi & paket bundling** untuk mendorong hunian di **{low_month}**."
                rec_type = "warning"
            else:
                rec = f"Terapkan **harga kompetitif & promosi agresif** mulai **{low_month}**."
                rec_type = "error"

            with st.expander(f"{icon} **{info['title']}** — Rata-rata prediksi: **{avg_f:.1f}%**", expanded=True):
                # Metrik
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("Prediksi Rata-rata", f"{avg_f:.1f}%")
                m2.metric("Puncak Hunian", peak_month)
                m3.metric("Hunian Terendah", low_month)
                mq_icon, mq_label = model_quality_badge(info.get("mape", float("nan")))
                m4.metric("Kualitas Model", mq_icon, delta=mq_label)


                # Grafik forecast
                info_d = dict(info)
                if villa in clean_occ_f:
                    info_d["monthly"] = clean_occ_f[villa].resample("MS").mean().dropna()
                st.plotly_chart(chart_forecast(info_d, fore), use_container_width=True)

                # Tabel detail
                fore_rows = []
                for date, val, lo, hi in zip(
                    fore["fore_mean"].index, fore["fore_mean"].values,
                    fore["fore_ci"].iloc[:, 0].values, fore["fore_ci"].iloc[:, 1].values,
                ):
                    ic, lb, _ = status_badge(val)
                    fore_rows.append({
                        "Bulan":           date.strftime("%b %Y"),
                        "Prediksi (%)":    f"{val:.1f}%",
                        "Rentang CI 90%":  f"{lo:.1f}% – {hi:.1f}%",
                        "Status":          f"{ic} {lb}",
                    })
                st.dataframe(pd.DataFrame(fore_rows), use_container_width=True, hide_index=True)

        # Perbandingan antar vila
        st.divider()
        section_header("Perbandingan Prediksi Antar Vila", "📊")
        fig_cmp = go.Figure()
        months_set = []
        for villa in selected_villas:
            if villa not in fore_info_all:
                continue
            fore = fore_info_all[villa]
            info = sarima_models[villa]
            color = villa_cfg.get(villa, {}).get("color", "#2563EB")
            months_set = [d.strftime("%b %Y") for d in fore["fore_mean"].index]
            fig_cmp.add_trace(go.Bar(
                name=info["title"],
                x=months_set, y=fore["fore_mean"].values,
                marker_color=color,
                text=[f"{v:.0f}%" for v in fore["fore_mean"].values],
                textposition="outside",
                hovertemplate=f"<b>{info['title']}</b><br>%{{x}}: %{{y:.1f}}%<extra></extra>",
            ))
        apply_base(fig_cmp,
            barmode="group",
            yaxis=dict(title="Prediksi Hunian (%)", range=[0, 120], ticksuffix="%", gridcolor="#F3F4F6"),
            height=360,
            legend=dict(bgcolor="rgba(255,255,255,0.9)", bordercolor="#E5E7EB", borderwidth=1),
            title=dict(text=f"Perbandingan Forecast +{FORECAST_STEPS} Bulan", font=dict(size=13, color="#1E3A5F")),
        )
        ...
        for villa in selected_villas:
            ...
            for date, val, lo, hi in zip(
                ...
                dl_rows.append({
                    ...
                    "Area": villa_cfg.get(villa, {}).get("area", "").title(),
                    "Bulan": date.strftime("%b %Y"),
                    "Prediksi (%)": round(val, 1),
                    "CI Lower (%)": round(lo, 1),
                    "CI Upper (%)": round(hi, 1),
                })
        if dl_rows:
            st.download_button("⬇️ Download Tabel Prediksi (CSV)",
                pd.DataFrame(dl_rows).to_csv(index=False).encode("utf-8"),
                "prediksi_hunian.csv", "text/csv")

    # ─── TAB: ANALISIS HARGA (User & Admin) ───
    harga_tab_idx = 1 if not is_admin else None  # untuk user, tab kedua
    ...
    if is_admin:
            ...
            for villa in selected_villas:
                ...
                with st.expander(label_exp, expanded=False):
                    ...
                    if info:
                        ...
                        c5.metric("RMSE Model", f"{rmse_v:.1f}%")
                    st.divider()
                    ...
                    with t3:
                        ...
                        st.caption(
                            f"Threshold: ±{conf:.3f} | "
                            "ACF truncate tajam → MA(q) | PACF truncate tajam → AR(p)"
                        )
        ...
        with tabs[3]:
            ...
            st.dataframe(pd.DataFrame([{
                ...
                "RMSE (%)": round(db_load_model(v).get("rmse", 0), 2) if db_load_model(v) else "—",
                "MAPE (%)": round(db_load_model(v).get("mape", 0), 2) if db_load_model(v) else "—",
            } for v in avail_tr]), use_container_width=True, hide_index=True)

            st.divider()
            c_sel, c_opt = st.columns([3, 1])
            with c_sel:
                train_sel = st.multiselect(
                    "Pilih Vila untuk Dilatih", avail_tr,
                    default=avail_tr,
            ...
            if st.button("🚀 Mulai Training", type="primary", use_container_width=True):
                ...
                if not to_train:
                    ...
                    st.success(f"Berhasil melatih **{len([r for r in results if '✅' in r.get('Status','')])}** dari {len(to_train)} vila.")
...
def main():
    ...
    with st.sidebar:
        ...
        # Filter vila (untuk halaman yang relevan)
